...
function Login() {
    ...
    const loginFunction = async (Name, Email, Password) => {
        try {
            const loginResponse = await axios.post("http://localhost:3000/Customer-Login/login", {
                Name,
                Email,
                Password
            }, { withCredentials: true });
            if (loginResponse.data.response) {
                toast("Welcome To The AutoX CarStore")
                return Navigate("/AutoX-Carstore");
            }
                ...
                <div className=" box-border flex w-[98%]  h-[550px]  justify-around items-center">
                    ...
                    <div data-aos="fade-right" className="  bg-black box-border flex flex-col  items-center  w-[35%] gap-[30%]  h-[520px]  ">
                        ...
                        <div className=" box-border w-[95%]  flex flex-col gap-5 items-center justify-center ">
                            ...
                            <input onChange={(e) => { setEmail(e.target.value) }} type="email" placeholder="Email" className="  text-white flex border-b-2   border-red-800 text-center bg-black outline-none w-[100%] h-[43px]"></input>
                            <input onChange={(e) => { setPassword(e.target.value) }} type="password" placeholder="Password" className="   text-white bg-black  border-b-2   border-red-800 flex text-center outline-none w-[100%] h-[43px]"></input>
                            <p className=" text-white">You have not account then <span className="underline cursor-pointer hover:text-red-700"><Link to="/AutoX-SignUpForm">SignUp</Link></span></p>
