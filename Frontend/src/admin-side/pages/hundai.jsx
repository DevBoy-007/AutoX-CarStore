...
function Hundia() {
    ...
    const display_Cars = async () => {
        try {
            const cars_List = await axios.get("http://localhost:3000/Hondia-DisplayCars/hondiadisplay", {
    ...
    const submit_Cars = async (Vin, Url, Make, Model, Year, Body, Color, Fuel, Price) => {
        try {
            ...
            const submit_Response = await axios.post("http://localhost:3000/Hondia-SubmitCars/hondiasubmit", formData, {
                headers: {
    ...
    const update_Car = async (Vin, Url, Make, Model, Year, Body, Color, Fuel, Price) => {
        try {
            ...
            formData.append('Fuel', Fuel);
            formData.append('Price', Price);
            const updata_Response = await axios.put("http://localhost:3000/Hondia-UpdateCars/hondiaupdate", formData, {
                headers: {
                    'Content-Type': 'multipart/form-data'
                }
            }, { withCredentials: true })
    ...
    const remove_Car = async (Vin) => {
        try {
            ...
            const delete_Response = await axios.delete("http://localhost:3000/Hondia-DeleteCars/hondiadelete", {
                params: {
                    Vin: Vin,
                }
            }, { withCredentials: true })
        ...
        <>
            <main className="  mb-[4%]  mt-[3%] flew flex-wrap w-[98%] m-[auto]">
                <div className=" flex  justify-center items-center text-[13px]  cursor-pointer text-black text-center   w-[100%] h-[50px]">
                    <h1 data-aos="zoom-in" onClick={() => { modelOpen1(true) }}>HYUNDAI SHOWROOM</h1>
                </div>
