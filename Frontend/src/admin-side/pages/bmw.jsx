...
function Bmw() {
  ...
  const display_Cars = async () => {
    try {
      const cars_List = await axios.get("http://localhost:3000/Bmw-DisplayCars/bmwdisplay", {
  ...
  const submit_Cars = async (Vin, Url, Make, Model, Year, Body, Color, Fuel, Price) => {
    try {
      ...
      const submit_Response = await axios.post("http://localhost:3000/Bmw-SubmitCars/bmwsubmit", formData, {
        headers: {
  ...
  const update_Car = async (Vin, Url, Make, Model, Year, Body, Color, Fuel, Price) => {
    try {
      ...
      formData.append('Fuel', Fuel);
      formData.append('Price', Price);
      const updata_Response = await axios.put("http://localhost:3000/Bmw-UpdateCars/bmwupdate", formData, {
        headers: {
          'Content-Type': 'multipart/form-data'
        }
      }, { withCredentials: true })
  ...
  const remove_Car = async (Vin) => {
    try {
      ...
      const delete_Response = await axios.delete("http://localhost:3000/Bmw-DeleteCars/bmwdelete", {
        params: {
          Vin: Vin,
        }
      }, { withCredentials: true })
    ...
    <>
      <main className="  mb-[4%]  mt-[3%] flew flex-wrap w-[98%] m-[auto]">
        <div className=" flex  justify-center items-center text-[13px]  cursor-pointer text-black text-center   w-[100%] h-[50px]">
          <h1 data-aos="zoom-in" onClick={() => { modelOpen1(true) }}> BMW SHOWROOM</h1>
        </div>
          ...
          <div className="  overflow-y-scroll w-[80%] h-screen bg-neutral-900">
            ...
            <div className="   w-[100%] mt-[1%] bg-stone-900">
              ...
              <div className=" flex justify-center space-x-80 items-center mt-[2%]  w-[100] h-[100px]  bg-neutral-900">
                ...
                <button onClick={() => { modelClose1(false) }} className=" shadow-2xl shadow-black text-white bg-black w-[150px] h-[50px]">Cancel</button>
              </div>
            </div>
          </div>
        </div>)}
          ...
          <div className=" overflow-y-scroll w-[80%] h-screen bg-black">
            ...
            <div className="   w-[100%] mt-[5%]  bg-neutral-900 ">
              ...
              <div className=" flex justify-center space-x-80 items-center mt-[2%]  w-[100] h-[100px]  bg-neutral-900">
                ...
                <button onClick={() => { modelClose2(false) }} className=" shadow-2xl shadow-black text-white bg-black w-[150px] h-[50px]">Cancel</button>
              </div>
            </div>
          </div>
        </div>)}
        ...
        <div className=" flex justify-center  gap-x-[3%] flex-wrap w-[100%]   mt-[1%]">
          {(car_list.length == 0 && <h1 className=" text-[25px] shadow-2xl shadow-black text-black">Bmw ShowRoom has No Car</h1>)}
          {
